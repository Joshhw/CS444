...
int ttywrite(int dev, char *buf, int nchar)
{
    ...
    if(devtab[dev].dvnum == 0){
      sleep(TTY0_OUT); /*queue is full, call sleep with 1 */
    } else if(devtab[dev].dvnum == 1){
      sleep(TTY1_OUT); /*queue is full, call sleep with 2 */
    }
    set_eflags(saved_eflags);
  }
    }
...
void irqinthandc(int dev)
{
	...
	if (queuecount( &tty->ebuf ))/* if there is char in echo Q output it*/
      ...
      if(devtab[dev].dvnum == 0){
        wakeup(TTY0_OUT); /*queue is not full, call wakeup with 1 */
      } else if(devtab[dev].dvnum == 1){
        wakeup(TTY1_OUT); /*queue is not full, call wakeup with 2 */
      }
	    kickout(baseport);		   /* on VMWare, restart ints */
	}
	else			/* all done transmitting */ {
	    outpt( baseport+UART_IER, UART_IER_RDI); /* shut down tx ints */
      debug_log("s"); /*shutdown tx int */
	}
    }
//  schedule();
}
